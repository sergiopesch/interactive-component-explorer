...
/**
 * Identify an electronic component from an image file.
 * Returns the best match with confidence, or null if no match meets the threshold.
 */
export async function identifyComponent(
  imagePath: string,
  threshold = 0.03
): Promise<IdentifyResult | null> {
  const results = await classify(imagePath, ALL_CANDIDATE_LABELS)
  ...
  return {
    ...
    reasoning: 'Identified via local CLIP model',
  }
}
